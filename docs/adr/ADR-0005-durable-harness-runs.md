...
- Filesystem serialization and symlink/containment checks are process-local. Linux `O_NOFOLLOW` protects final-component event/document opens. Artifact validation opens a workspace-root handle, walks and snapshots each parent with `lstat`/`realpath`, opens the final file with `O_NOFOLLOW`, and then compares handle/final/parent identity again. Batch atomicity is defined at the single append syscall/single-writer boundary: one process-local queued append either rejects before cache publication or commits the complete payload. This design assumes a trusted single writer; hostile concurrent directory replacement, partial low-level filesystem writes, or multi-process writers require a future directory-handle/locking or transactional design.
- Retention is directory-granular and must be configured operationally.
- Child lineage is immutable metadata derived by `RunService.createChildRun`: root and depth are server-owned, depth is capped at four, and parent projections contain at most 100 shallow summaries. Child completion never completes its parent; configured required children must independently have `verification.passed` evidence.
- Checkpoints are bounded metadata-only records, atomically written beneath `checkpoints/` and referenced by append-only events. Resume appends a new checkpoint-mode retry attempt and never rewrites history or launches execution by itself.
- Retention is opt-in and dry-run by default. Destructive pruning requires an injected enable flag, protects active runs and parents of retained children, and rejects symlinked run directories. No scheduler is installed.
- Export is bounded JSON and structurally redacts credentials, cwd, and internal paths. Truncation is explicit.
