...
async function revalidateExecutable(recipe, fsOps) {
  ...
  const current={dev:stat.dev,ino:stat.ino,mtimeMs:stat.mtimeMs,size:stat.size};
  for (const key of ['dev','ino','mtimeMs','size']) if (current[key] !== recipe.executableIdentity[key]) throw new TypeError('verification executable identity changed');
}
...
async function runCheck(recipe, workspace, options) {
  ...
  await revalidateExecutable(recipe, options.fsOps);
...
async function validateArtifact(workspace, artifact, fsOps) {
  const target = path.resolve(workspace, artifact);
  ...
  try {
    ...
    if (real !== path.resolve(workspace) && !real.startsWith(root)) return { path: artifact, valid: false, reason: 'outside_workspace' };
    ...
    return { path: artifact, valid: false, reason: error.code || 'unreadable' };
